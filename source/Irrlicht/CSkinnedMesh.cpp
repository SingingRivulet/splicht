	...
	AnimateNormals(true), HardwareSkinning(false)
{
	...
	animationOverrideCallback=NULL;
}
...
void CSkinnedMesh::animateMesh(f32 frame, f32 blend)
{
	...
	for (u32 i=0; i<AllJoints.size(); ++i)
	{
		...
		if(animationOverrideCallback && animationOverrideCallback->getFrameData(frame, joint,
...
template <class T>
void FindKey(const core::array<T>& keys, f32 targetFrame, int& posIndex, int& hint) {
    u32 firstIndex = 0;
	...
	if (joint->UseAnimationFrom)
	{
		...
		if (ScaleKeys.size())
		{
			...
			if (foundScaleIndex==-1)
			{
				...
				//	}
				//}
				FindKey(ScaleKeys, frame, foundScaleIndex, scaleHint);
			}
		...
		if (RotationKeys.size())
		{
			...
			if (foundRotationIndex==-1)
			{
				...
				//	}
				//}
				FindKey(RotationKeys, frame, foundRotationIndex, rotationHint);
			}
